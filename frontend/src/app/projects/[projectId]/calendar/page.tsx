...
import {
  ...
  Plus, Clock, Pencil, Trash2, Image, Video, FileText, Link, Upload, ExternalLink,
} from 'lucide-react'
...
export default function CalendarPage() {
  ...
  function syncEntries(next: CalendarEntry[]) {
    const sorted = [...next].sort((a, b) => a.date.localeCompare(b.date))
    setEntries(sorted)
    setCalendarCache(cacheKey, sorted)
  }

  function handleEntriesAdded(newEntries: CalendarEntry[]) {
    syncEntries([...entries, ...newEntries])
    setAddDate(null)
  }
  ...
  async function handleStatusChange(entry: CalendarEntry, newStatus: string) {
    try {
      ...
      const next = entries.map((e) => e.id === entry.id ? { ...e, status: newStatus } : e)
      syncEntries(next)
      setSelectedEntry((e) => e?.id === entry.id ? { ...e, status: newStatus } : e)
    } catch { toast.error('Failed to update status') }
  }

  async function handleSaveEntry(entry: CalendarEntry, updates: Partial<CalendarEntry>) {
    try {
  ...
  async function handleDeleteEntry(entry: CalendarEntry) {
    try {
      ...
      syncEntries(entries.filter((e) => e.id !== entry.id))
    ...
    <div className="flex flex-col flex-1 overflow-hidden" onClick={() => setOverflowDay(null)}>
      ...
      {selectedEntry && (
        <EntryDetailModal
          ...
          onSave={(updates) => handleSaveEntry(selectedEntry, updates)}
  ...
  return (
    <button onClick={onClick} className="w-full text-left">
      <div className={cn('flex items-center gap-1 rounded-sm px-1 py-0.5 hover:brightness-95 transition-all', pillClass)}>
        ...
        {entry.media_type === 'image' && <Image className="w-2 h-2 shrink-0 opacity-60" />}
        {entry.media_type === 'video' && <Video className="w-2 h-2 shrink-0 opacity-60" />}
        {entry.media_type === 'blog'  && <FileText className="w-2 h-2 shrink-0 opacity-60" />}
...
function OverflowPopover({ entries, onSelect, onClose }: {
  ...
  return (
    ...
    >
      ...
      {entries.map((entry) => <EntryChip key={entry.id} entry={entry} onClick={() => onSelect(entry)} />)}
    </div>
  )
}

// ---------------------------------------------------------------------------
// Media section (shared between Add + Edit modals)
// ---------------------------------------------------------------------------

type MediaTab = 'none' | 'image' | 'video' | 'blog'
...
function MediaSection({
  ...
  const tabs: { id: MediaTab; icon: React.ReactNode; label: string }[] = [
    { id: 'none',  icon: null,                             label: 'None' },
    { id: 'image', icon: <Image className="w-3 h-3" />,    label: 'Image' },
    { id: 'video', icon: <Video className="w-3 h-3" />,    label: 'Video' },
    { id: 'blog',  icon: <FileText className="w-3 h-3" />, label: 'Blog' },
  ]

  return (
    <div className="space-y-2">
      <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Media attachment</label>
      ...
      <div className="flex gap-1">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => { setMediaTab(t.id); setMediaUrl('') }}
            className={cn(
              'flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium transition-colors',
              mediaTab === t.id ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground',
            )}
          >
            {t.icon}{t.label}
          </button>
        ...
        <div>
          {mediaUrl ? (
            <div className="relative rounded-lg overflow-hidden border border-border">
              ...
              <img src={mediaUrl} alt="Media preview" className="w-full max-h-40 object-cover" />
              <button
                onClick={() => setMediaUrl('')}
...
function EntryDetailModal({ entry, projectId, onClose, onStatusChange, onSave, onDelete }: {
  ...
  onSave: (updates: Partial<CalendarEntry>) => Promise<void>
  onDelete: () => void
}) {
  ...
  const [mediaTab, setMediaTab]               = useState<MediaTab>(
    (entry.media_type as MediaTab | undefined) ?? 'none'
  )
  const [mediaUrl, setMediaUrl]               = useState(entry.media_url ?? '')
  const [uploading, setUploading]             = useState(false)
  const [saving, setSaving]                   = useState(false)

  // Sync if parent updates entry (e.g. status change from outside edit mode)
  useEffect(() => {
    if (!editing) {
      ...
      setEditTags(entry.hashtags.join(' '))
      setEditStatus(entry.status)
      setMediaTab((entry.media_type as MediaTab | undefined) ?? 'none')
  ...
  async function save() {
    ...
    try {
      await onSave({
        ...
        type: editType,
        content_format: TYPE_FORMAT_MAP[editType.charAt(0).toUpperCase() + editType.slice(1)] ?? 'post',
        date: editDate,
        time: editTime || undefined,
        content: editContent,
        hashtags: parseHashtags(editTags),
        status: editStatus,
        media_url: mediaUrl || null,
        media_type: mediaUrl ? mediaTab : null,
      })
      setEditing(false)
    } finally {
      setSaving(false)
          ...
          {editing ? (
            ...
            <>
                  ...
                  {PLATFORMS.map((p) => (
                    <button key={p} onClick={() => setEditPlatform(p)}
                      className={cn('flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium transition-colors',
                        editPlatform === p ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground')}>
                      {editPlatform === p && <div className={cn('w-1.5 h-1.5 rounded-full', PLATFORM_DOT[p])} />}
                      {p}
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {CONTENT_TYPES.map((t) => (
                    <button key={t} onClick={() => setEditType(t.toLowerCase())}
                      className={cn('px-2.5 py-1 rounded-md text-xs font-medium transition-colors',
                        editType === t.toLowerCase() ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground')}>
                      {t}
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <input value={editTags} onChange={(e) => setEditTags(e.target.value)} placeholder="marketing launch tips"
                  className="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary" />
              </div>
              ...
              <MediaSection
                projectId={projectId}
                mediaTab={mediaTab} setMediaTab={setMediaTab}
                  ...
                  {STATUSES.map((s) => (
                    <button key={s} onClick={() => setEditStatus(s)}
                      className={cn('flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-medium capitalize border transition-colors',
                        editStatus === s ? STATUS_PILL[s] + ' border-current/20' : 'bg-background border-border text-muted-foreground hover:text-foreground')}>
                      {editStatus === s && <Check className="w-2.5 h-2.5" />}{s}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <>
              ...
              {/* Content */}
              <div>
                <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide block mb-2">Content</span>
              ...
              {entry.media_url && (
                <div>
                  <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide block mb-2">Media</span>
                  {entry.media_type === 'image' ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={entry.media_url} alt="Attached media" className="w-full max-h-48 object-cover rounded-lg border border-border" />
                  ) : (
                    <a href={entry.media_url} target="_blank" rel="noopener noreferrer"
                      ...
                      {entry.media_type === 'video' ? <Video className="w-4 h-4 text-muted-foreground" /> : <FileText className="w-4 h-4 text-muted-foreground" />}
...
// ---------------------------------------------------------------------------
// Add Entry modal (multi-platform)
// ---------------------------------------------------------------------------
...
function AddEntryModal({ projectId, defaultDate, onClose, onCreated }: {
  ...
  defaultDate: string
  onClose: () => void
  onCreated: (entries: CalendarEntry[]) => void
}) {
  ...
  async function handleSubmit() {
    ...
    try {
      const payload = {
        ...
        media_type: mediaUrl ? mediaTab : undefined,
      }
      const created = await Promise.all(
        platforms.map((platform) => api.calendar.createEntry(projectId, { ...payload, platform }))
      )
      toast.success(
        platforms.length === 1
          ? 'Entry added to calendar'
          : `Added to ${platforms.length} platforms`
      )
      onCreated(created)
    } catch (err) {
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
              Platforms
              <span className="ml-1.5 normal-case font-normal text-muted-foreground/70">(select multiple)</span>
            </label>
            <div className="flex flex-wrap gap-1.5">
              {PLATFORMS.map((p) => {
            ...
            {platforms.length > 1 && (
              <p className="text-[10px] text-muted-foreground">
                Will create {platforms.length} separate entries — one per platform.
              </p>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
              Hashtags <span className="font-normal normal-case">(space-separated, optional)</span>
            </label>
            <input value={hashtags} onChange={(e) => setHashtags(e.target.value)} placeholder="marketing socialmedia launch"
              className="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary" />
          </div>
              ...
              {STATUSES.map((s) => (
                <button key={s} onClick={() => setStatus(s)}
                  className={cn('px-2.5 py-1 rounded-full text-[10px] font-medium capitalize border transition-colors',
                    status === s ? STATUS_PILL[s] + ' border-current/20' : 'bg-background border-border text-muted-foreground hover:text-foreground')}>
                  {s}
                </button>
              ))}
            </div>
        ...
        <div className="px-5 py-4 border-t border-border shrink-0">
          <button onClick={handleSubmit} disabled={saving || uploading || !content.trim()}
            ...
            {saving
              ? 'Adding…'
              : platforms.length > 1
                ? `Add to ${platforms.length} platforms`
                : 'Add to calendar'}
          </button>
        </div>
      </div>
  ...
  async function handleGenerate() {
    ...
    try {
      await api.calendar.generate(projectId, {
        platforms, period: `${MONTH_NAMES[month]} ${year}`,
        posts_per_week: postsPerWeek, goals: themes.trim() || undefined,
      })
      toast.success('Calendar generated!')
      onDone()
    } catch (err) { toast.error('Generation failed'); console.error(err) }
            ...
            <div className="flex gap-1.5">
              {[1, 2, 3, 5, 7].map((n) => (
                <button key={n} onClick={() => setPostsPerWeek(n)}
                  className={`w-9 h-9 rounded-md text-sm font-medium transition-colors ${postsPerWeek === n ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}>
                  {n}
                </button>
              ))}
            </div>
