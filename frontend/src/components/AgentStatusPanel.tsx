...
const statusColor: Record<string, string> = {
  ...
  success: "text-emerald-700 border-emerald-500/20 bg-emerald-500/10 dark:text-emerald-300",
  completed: "text-emerald-700 border-emerald-500/20 bg-emerald-500/10 dark:text-emerald-300",
...
export default function AgentStatusPanel({ agents, collapsed, onToggle, totalTime, totalSources }: Props) {
  const allDone = agents.every((a) => ["done", "success", "completed", "failed", "partial"].includes(a.status));
    ...
    return (
      ...
      >
        ...
        <span>
          ...
          {totalSources ? ` · ${totalSources} sources analyzed` : ""}
        </span>
      </motion.button>
    );
  }
          ...
          <CardContent className="p-0">
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              ...
              <div className="flex items-center gap-2">
                                <Badge variant={allDone ? "default" : "info"}>{allDone ? "Complete" : "Running"}</Badge>
            ...
            <div className="space-y-3 p-5">
              {agents.map((agent, i) => {
                const Icon = statusIcon[agent.status] || Activity;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`flex h-10 w-10 items-center justify-center rounded-2xl border ${statusColor[agent.status] || statusColor.queued}`}>
                          ...
                          <div
                            className={`h-1.5 rounded-full transition-all duration-500 ${
                              agent.status === "running"
                                ? "w-3/5 bg-primary"
                                : ["done", "success", "completed"].includes(agent.status)
